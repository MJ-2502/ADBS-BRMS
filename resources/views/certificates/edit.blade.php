...
@section('content')
<h1 class="text-xl font-semibold text-slate-800 dark:text-white">Edit certificate request</h1>
<form method="POST" action="{{ route('certificates.update', $certificate) }}" class="mt-6 grid gap-4 rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-800/50">
    @csrf
    @method('PUT')
        ...
        <div>
            ...
            <select name="resident_id" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 dark:border-slate-700 dark:bg-slate-900 dark:text-white" required>
                ...
                @foreach($residents as $resident)
                    <option value="{{ $resident->id }}" @selected(old('resident_id', $certificate->resident_id) == $resident->id)>{{ $resident->full_name }}</option>
                @endforeach
            </select>
        </div>
    @endif
    <div class="grid gap-4 sm:grid-cols-2">
        <div>
            ...
            <select id="certificate_type_edit" name="certificate_type" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 dark:border-slate-700 dark:bg-slate-900 dark:text-white" required>
            ...
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Fee updates when you change the certificate type.</p>
        </div>
        <div>
            <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Purpose</label>
            <input type="text" name="purpose" value="{{ old('purpose', $certificate->purpose) }}" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 dark:border-slate-700 dark:bg-slate-900 dark:text-white" required>
        </div>
    </div>
    <div class="grid gap-4 sm:grid-cols-2">
        <div>
            <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Remarks</label>
            <textarea name="remarks" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 dark:border-slate-700 dark:bg-slate-900 dark:text-white" rows="3">{{ old('remarks', $certificate->remarks) }}</textarea>
        </div>
