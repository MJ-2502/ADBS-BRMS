...
@section('content')
<h1 class="text-xl font-semibold text-slate-800 dark:text-white">Edit household</h1>
<form method="POST" action="{{ route('households.update', $household) }}" class="mt-6 space-y-5 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 dark:border-slate-800 dark:bg-slate-800/50">
    @csrf
    @method('PUT')
    <div>
        <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Household number</label>
        <input type="text" name="household_number" value="{{ old('household_number', $household->household_number) }}" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-base dark:border-slate-700 dark:bg-slate-900 dark:text-white" required>
    </div>
    <div>
        <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Address</label>
        <input type="text" name="address_line" value="{{ old('address_line', $household->address_line) }}" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-base dark:border-slate-700 dark:bg-slate-900 dark:text-white" required>
    </div>
    <div class="grid gap-4 sm:grid-cols-2">
        <div>
            <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Purok</label>
            <input type="text" name="purok" value="{{ old('purok', $household->purok) }}" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-base dark:border-slate-700 dark:bg-slate-900 dark:text-white">
        </div>
        <div>
            <label class="text-sm font-medium text-slate-600 dark:text-slate-300">Zone</label>
            <input type="text" name="zone" value="{{ old('zone', $household->zone) }}" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-base dark:border-slate-700 dark:bg-slate-900 dark:text-white">
        </div>
    </div>
    <div class="grid gap-4 sm:grid-cols-2">
        <div>
    ...
    <div class="flex flex-wrap items-center justify-end gap-3">
        ...
        <button class="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 dark:bg-emerald-500 dark:hover:bg-emerald-600">Update household</button>
    </div>
</form>
@endsection
